...
export const ButtonSmall = ({
  ...
  <button
    ...
    className={classNames(
      'hover:bg-emerald-300/50 cursor-pointer p-1 rounded text-slate-600 dark:text-slate-100',
      className
    )}
  >
    {children}
...
const PlaybackController = ({ state, dispatch, reset }) => {
  const { audioId, setAudioId } = useQuranReader((state) => ({
    ...
    <>
      <div className="flex items-center justify-center">
        <div className="flex w-[16rem] justify-around relative">
          ...
          >
            <RepeatIcon
              className={classNames('h-6', {
                'text-emerald-500': state.isRepeat,
              })}
            />
          </ButtonSmall>
          <ButtonSmall onClick={goToPrevSurah} className="">
          ...
          <ButtonSmall onClick={goToNextSurah} className="transform rotate-180">
            <RewindIcon className="h-6" />
          </ButtonSmall>
          <PlaybackOption onClickReset={reset} />
        </div>
      </div>
    </>
  );
